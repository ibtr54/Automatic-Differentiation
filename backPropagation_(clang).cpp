...
//This code for some reason only runs using clang compiler but crash using g++.
...
public:
    ...
    template <typename T>
    Variable operator+(const T& other) const {
...
void SingleNeuron() {
    ...
    for (int epoch = 0; epoch < epochs; ++epoch) {
        ...
        Variable cloneW(w.value,w.parents,w.op);
        cloneW.gradient = w.gradient;
        Variable cloneB(b.value, b.parents, b.op);
        cloneB.gradient = b.gradient;
